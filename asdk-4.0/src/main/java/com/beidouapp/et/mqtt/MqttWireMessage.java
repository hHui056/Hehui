...
public abstract class MqttWireMessage {
	...
	// payload = new byte[0];
	// }
	// MultiByteArrayInputStream mbais = new
	// MultiByteArrayInputStream(data.getHeaderBytes(), data.getHeaderOffset(),
	// data.getHeaderLength(), payload, data.getPayloadOffset(),
	// data.getPayloadLength());
	// return createWireMessage(mbais);
	// }

	public static MqttWireMessage createWireMessage(byte[] bytes) throws IOException {
		ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
		return createWireMessage(bais);
	}

	private static MqttWireMessage createWireMessage(InputStream inputStream) throws IOException {
		try {
...
//			System.out.println("接收到消息类型：" + type);
//			System.err.println("接收到消息info:" + info);
//			System.out.println("接收到消息data:" + HexUtil.printHexString(data));
	...
	protected byte[] encodeMessageId() throws IOException {
		try {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			DataOutputStream dos = new DataOutputStream(baos);
	 ...
	 * bytes and then written to the DataOutputStream.
	 * 
	 * @link{DataOutputStream#writeUFT(String) should be no longer used.
	 * @link{DataOutputStream#writeUFT(String) does not correctly encode UTF-16
	 ...
	 * 
	 * @link(DataInoutStream#readUTF()) should be no longer used, because
	 * @link(DataInoutStream#readUTF()) does not decode UTF-16 surrogate
