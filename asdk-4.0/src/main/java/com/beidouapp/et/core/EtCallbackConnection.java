 ...
 */
public class EtCallbackConnection {
    ...
    public EtCallbackConnection(EtMQTT mqtt) {
        this.mqtt = mqtt;
    ...
    void createTransport(final Callback<Transport> onConnect) throws Exception {
        ...
        if (mqtt.blockingExecutor == null) {
            mqtt.blockingExecutor = EtMQTT.getBlockingThreadPool();
        }
            ...
            heartBeatMonitor.setOnKeepAlive(new Task() {
                ...
                public void run() {
                    ...
                    if (!disconnected && pingedAt == 0) {
                        ...
                        if (EtCallbackConnection.this.transport.offer(encoded)) {
                            mqtt.tracer.onSend(encoded);
                            final long now = System.currentTimeMillis();
                            final long suspends = suspendChanges.get();
                            pingedAt = now;
                            queue.executeAfter(EtCallbackConnection.this.mqtt.getKeepAlive(), TimeUnit.SECONDS, new Task() {
                                @Override
                                public void run() {
                                    if (now == pingedAt) {
    ...
    public EtCallbackConnection refiller(Runnable refiller) {
        queue.assertExecuting();
        this.refiller = refiller;
        return this;
    }

    public EtCallbackConnection listener(Listener listener) {
