...
public class ProtoCommon {
    ...
    public static class RecvHeaderInfo {
        ...
        public RecvHeaderInfo(byte errno, byte[] header) {
            this.errno = errno;
            this.header = header;
    ...
    public static final byte STORAGE_PROTO_CMD_DOWNLOAD_FILE_CHECK = 116;// 带验证的下载文件命令.
    public static final byte STORAGE_PROTO_CMD_DELETE_FILE_CHECK = 117; // 带验证的删除文件命令.
    public static final byte STORAGE_PROTO_CMD_UPLOAD_FILE_CHECK = 118; // 带验证的上传文件命令.
    ...
    public static final int ENCRYPT_LEN = 32; // 加密串32位.
    public static final int UID_LEN = 34; // UID34位.
    public static final int RANDOM_LEN = 10;// 随机数10位.
...
//        }
        ...
        if (pkg_len == 0) {//无报文体，仅仅验证
            return new RecvHeaderInfo((byte) 0, header);
        }
        if (pkg_len < 0) {
     ...
     * @param ext_name        the extension name of slave filename, null for same as the
    ...
    public static String genSlaveFilename(String master_filename,
        ...
        if (master_filename.length() < 28 + FDFS_FILE_EXT_NAME_MAX_LEN) {
            throw new MyException("master filename \"" + master_filename
                    + "\" is invalid");
        }
        ...
        if (true_ext_name.length() == 0 && prefix_name.equals("-m")) {
            throw new MyException("prefix_name \"" + prefix_name
                    + "\" is invalid");
        }
