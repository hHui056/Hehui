...
import static org.csource.fastdfs.ProtoCommon.PROTO_HEADER_STATUS_INDEX;
...
public class StorageClient {
     ...
     * @param file_ext_name  file ext name, do not include dot(.), null to extract ext name
    ...
    public String[] upload_file_with_check(String group_name, long file_size,
                                           UploadCallback callback, String file_ext_name,
                                           NameValuePair[] meta_list, Map<String, Object> params)
            throws IOException, MyException {
        final String master_filename = null;
        final String prefix_name = null;

        return this.do_upload_file_with_check(
                ProtoCommon.STORAGE_PROTO_CMD_UPLOAD_FILE_CHECK, group_name,
                master_filename, prefix_name, file_ext_name, file_size,
                callback, meta_list, params);
    }
    ...
    protected String[] do_upload_file_with_check(byte cmd, String group_name,
                                                 String master_filename, String prefix_name, String file_ext_name,
                                                 long file_size, UploadCallback callback, NameValuePair[] meta_list,
                                                 Map<String, Object> params) throws IOException, MyException {
        byte[] header;
        byte[] ext_name_bs;// 文件扩展名字节数组.
        String new_group_name;
        String remote_filename;
        boolean bNewConnection;
        Socket storageSocket;
        byte[] sizeBytes; // 文件本身长度的字节数组表示.
        byte[] hexLenBytes;
        byte[] masterFilenameBytes;
        boolean bUploadSlave;
        int offset = 0;
        long body_len;
        byte[] encryptBytes = new byte[ProtoCommon.ENCRYPT_LEN]; // iLink的encrypt字节数组.
        byte[] uidBytes = new byte[ProtoCommon.UID_LEN]; // iLink的UID字节数组.
        byte[] randomBytes = new byte[ProtoCommon.RANDOM_LEN]; // iLink的Random字节数组.
        Arrays.fill(encryptBytes, (byte) 0);
        Arrays.fill(uidBytes, (byte) 0);
        Arrays.fill(randomBytes, (byte) 0);

        byte[] tempEncryptBytes = ((String) params.get("encrypt"))
                .getBytes(ClientGlobal.g_charset);
        byte[] tempUidBytes = ((String) params.get("uid"))
                .getBytes(ClientGlobal.g_charset);
        byte[] tempRandomBytes = ((String) params.get("random"))
                .getBytes(ClientGlobal.g_charset);

        System.arraycopy(tempEncryptBytes, 0, encryptBytes, 0, tempEncryptBytes.length);
        System.arraycopy(tempUidBytes, 0, uidBytes, 0, tempUidBytes.length);
        System.arraycopy(tempRandomBytes, 0, randomBytes, 0, tempRandomBytes.length);
        ...
        try {
            ...
            OutputStream out = storageSocket.getOutputStream();
            header = ProtoCommon.packHeader(cmd, body_len - file_size, (byte) 0);
            byte[] wholePkg = new byte[(int) (header.length + body_len - file_size)];
            System.arraycopy(header, 0, wholePkg, 0, header.length);
            offset = header.length;

            System.arraycopy(encryptBytes, 0, wholePkg, offset, encryptBytes.length);
            offset += encryptBytes.length;

            System.arraycopy(uidBytes, 0, wholePkg, offset, uidBytes.length);
            offset += uidBytes.length;

            System.arraycopy(randomBytes, 0, wholePkg, offset, randomBytes.length);
            offset += randomBytes.length;

            System.arraycopy(sizeBytes, 0, wholePkg, offset, sizeBytes.length);
            ...
            out.write(wholePkg);
            //验证
            ProtoCommon.RecvHeaderInfo headerInfo = ProtoCommon.recvHeader(storageSocket.getInputStream(), ProtoCommon.STORAGE_PROTO_CMD_RESP, -1);
            if (headerInfo.header != null && headerInfo.header[PROTO_HEADER_STATUS_INDEX] != (byte) 0) {
                int code = headerInfo.header[PROTO_HEADER_STATUS_INDEX] & 0xFF;
                String errormsg = "";
                switch (code) {
                    case 190:
                        errormsg = "http server error";
                        break;
                    case 191:
                        errormsg = "token invalid";
                        break;
                    case 192:
                        errormsg = "fileSize too long";
                        break;
                    case 193:
                        errormsg = "beyond the daily total length";
                        break;
                    case 194:
                        errormsg = "unkonw error";
                        break;
                    case 195:
                        errormsg = "repeat request";
                        break;
                    case 196:
                        errormsg = "api invalid";
                        break;
                    default:
            ...
            if ((this.errno = (byte) callback.send(out)) != 0) {//验证通过，上传文件
                return null;
            }
     ...
     * @throws MyException
     */
    public int delete_file_with_check(String group_name,
                                      String remote_filename, Map<String, Object> params)
            throws IOException, MyException {
        boolean bNewConnection = this.newUpdatableStorageConnection(group_name,
                remote_filename);
        Socket storageSocket = this.storageServer.getSocket();
        String appKey = (String) params.get("encrypt");
        String uid = (String) params.get("uid");
        String random = (String) params.get("random");
        try {
            this.send_package_check(
                    ProtoCommon.STORAGE_PROTO_CMD_DELETE_FILE_CHECK,
                    group_name, remote_filename, appKey, uid, random);
            ProtoCommon.RecvPackageInfo pkgInfo = ProtoCommon.recvPackage(
                    storageSocket.getInputStream(),
                    ProtoCommon.STORAGE_PROTO_CMD_RESP, 0);
            this.errno = pkgInfo.errno;
    ...
    public int download_file_with_check(String group_name, String remote_filename, long file_offset, long download_bytes, String local_filename, Map<String, Object> params)
            throws IOException, MyException {
        ...
        try {
            ...
            try {
                this.errno = 0;
                this.send_download_package_check(group_name, remote_filename, file_offset, download_bytes, params);
                InputStream in = storageSocket.getInputStream();
                header = ProtoCommon.recvHeader(in,
                        ProtoCommon.STORAGE_PROTO_CMD_RESP, -1);
                this.errno = header.errno;
    ...
    public int download_file(String group_name, String remote_filename,
        ...
        try {
            ProtoCommon.RecvHeaderInfo header;
            this.send_download_package_check(group_name, remote_filename,
                    file_offset, download_bytes, params);
     ...
     * @throws IOException
     */
    protected void send_package_check(byte cmd, String group_name,
                                      String remote_filename, String encrypt, String uid, String random)
            throws IOException {
        byte[] header;
        byte[] groupBytes;
        byte[] filenameBytes;
        ...
        byte[] tempEncryptBytes = encrypt.getBytes(ClientGlobal.g_charset);
        byte[] tempUidBytes = uid.getBytes(ClientGlobal.g_charset);
        byte[] tempRandomBytes = random.getBytes(ClientGlobal.g_charset);

        System.arraycopy(tempEncryptBytes, 0, encryptBytes, 0,
                tempEncryptBytes.length);
        System.arraycopy(tempUidBytes, 0, uidBytes, 0, tempUidBytes.length);
        System.arraycopy(tempRandomBytes, 0, randomBytes, 0,
                tempRandomBytes.length);
        ...
        long size = encryptBytes.length + uidBytes.length + randomBytes.length
                + groupBytes.length + filenameBytes.length;
        header = ProtoCommon.packHeader(cmd, size, (byte) 0);

        int len = 0;
        byte[] wholePkg = new byte[header.length + (int) size];
        System.arraycopy(header, 0, wholePkg, 0, header.length);
        len += header.length;

        System.arraycopy(encryptBytes, 0, wholePkg, len, encryptBytes.length);
        len += encryptBytes.length;

        System.arraycopy(uidBytes, 0, wholePkg, len, uidBytes.length);
        len += uidBytes.length;

        System.arraycopy(randomBytes, 0, wholePkg, len, randomBytes.length);
        len += randomBytes.length;

        System.arraycopy(groupBytes, 0, wholePkg, len, groupBytes.length);
        len += groupBytes.length;

        System.arraycopy(filenameBytes, 0, wholePkg, len, filenameBytes.length);
        this.storageServer.getSocket().getOutputStream().write(wholePkg);
    }
    ...
    protected void send_download_package(String group_name, String remote_filename, long file_offset, long download_bytes)
        ...
        System.arraycopy(filenameBytes, 0, wholePkg, header.length + bsOffset.length + bsDownBytes.length + groupBytes.length,
                filenameBytes.length);
        this.storageServer.getSocket().getOutputStream().write(wholePkg);
    }

    protected void send_download_package_check(String group_name, String remote_filename, long file_offset, long download_bytes, Map<String, Object> params) throws IOException {
        byte[] header;
        byte[] bsOffset; // 文件的本次起始偏移量.
        byte[] bsDownBytes; // 文件下载的字节数组.
        byte[] groupBytes;
        byte[] filenameBytes;
        byte[] bs;
        ...
        byte[] tempEncryptBytes = ((String) params.get("encrypt")).getBytes(ClientGlobal.g_charset);
        byte[] tempUidBytes = ((String) params.get("uid")).getBytes(ClientGlobal.g_charset);
        byte[] tempRandomBytes = ((String) params.get("random")).getBytes(ClientGlobal.g_charset);
        ...
        System.arraycopy(bs, 0, groupBytes, 0, groupLen);
        int index = 0;
        header = ProtoCommon.packHeader(ProtoCommon.STORAGE_PROTO_CMD_DOWNLOAD_FILE_CHECK,
                bsOffset.length + bsDownBytes.length + groupBytes.length
                        + filenameBytes.length + encryptBytes.length
                        + uidBytes.length + randomBytes.length, (byte) 0);
        byte[] wholePkg = new byte[header.length + bsOffset.length
                + bsDownBytes.length + groupBytes.length + filenameBytes.length
                + encryptBytes.length + uidBytes.length + randomBytes.length];
        System.arraycopy(header, 0, wholePkg, 0, header.length);
        index += header.length;

        System.arraycopy(encryptBytes, 0, wholePkg, index, encryptBytes.length);
        index += encryptBytes.length;

        System.arraycopy(uidBytes, 0, wholePkg, index, uidBytes.length);
        index += uidBytes.length;

        System.arraycopy(randomBytes, 0, wholePkg, index, randomBytes.length);
        index += randomBytes.length;

        System.arraycopy(bsOffset, 0, wholePkg, index, bsOffset.length);
        index += bsOffset.length;

        System.arraycopy(bsDownBytes, 0, wholePkg, index, bsDownBytes.length);
        index += bsDownBytes.length;

        System.arraycopy(groupBytes, 0, wholePkg, index, groupBytes.length);
        index += groupBytes.length;

        System.arraycopy(filenameBytes, 0, wholePkg, index, filenameBytes.length);
        this.storageServer.getSocket().getOutputStream().write(wholePkg);
    }
}
